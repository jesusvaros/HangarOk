// supabase/functions/reject-review/index.ts
...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      return new Response(`Error al enviar email (network): ${err instanceof Error ? err.message : String(err)}`, { status: 500, headers: corsHeaders });
    }

    if (!response.ok) {
      const errorText = await response.text();
      return new Response(`Error al enviar email (${response.status}): ${errorText}`, { status: 500, headers: corsHeaders });
    }
