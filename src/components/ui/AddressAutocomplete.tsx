      ...
      <div className="flex space-x-2">
        ...
        <div className={`relative ${showNumberField ? 'w-3/4' : 'w-full'}`}>
          ...
          {results.length > 0 && isFocused && (
            <ul
              className={`absolute  left-0 right-0 z-50 mt-1 max-h-60 overflow-y-auto rounded-b-lg bg-white shadow-lg ${hideLabel ? 'top-[48px]' : 'top-[80px]'}`}
            >
              {results.map((result) => {
        ...
        {showNumberField && (
          <div className="w-1/4">
              ...
              onBlur={(e) => {
                ...
                if (validateNumber && selectedAddress && newNumber.trim() !== '') {
                  ...
                  if (street) {
                    ...
                    fetch(
                      `https://api.opencagedata.com/geocode/v1/json?q=${encodeURIComponent(validationQuery)}&key=${import.meta.env.VITE_OPENCAGE_API_KEY}&language=es&countrycode=es&limit=1&no_annotations=1`,
                    )
            ...
            />
            {numberError && <p className="mt-1 text-xs text-red-500">Número no coincide</p>}
          </div>
        )}
      </div>
    </div>
