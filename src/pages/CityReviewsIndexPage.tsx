      ...
      <main className="mx-auto max-w-6xl px-6 pt-28 pb-24">
        ...
        {loading ? (
          <section className="rounded-2xl border border-gray-200 bg-white p-12 text-center shadow-sm">
            <p className="text-gray-600">Loading cities with reviews…</p>
          </section>
        ) : null}
        ...
        {!loading && totalCities === 0 ? (
          <section className="rounded-2xl border border-gray-200 bg-white p-12 text-center shadow-sm">
            <h2 className="text-xl font-semibold text-gray-900">
              No public city reviews yet
            </h2>
            <p className="mt-3 text-gray-600">
              We’re moderating new submissions. In the meantime, open the map to see every experience published so far.
            </p>
            <Link
              to="/map"
        ...
        {!loading && totalCities > 0 ? (
          <section>
            <div className="mb-6 flex flex-wrap items-center gap-3 text-sm text-gray-600">
              <span>
                {totalCities} {totalCities === 1 ? 'city with reviews' : 'cities with reviews'}
              </span>
              <span>•</span>
              <span>
                {totalReviews} {totalReviews === 1 ? 'review published' : 'reviews published'}
              </span>
            </div>
